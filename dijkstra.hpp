...
{
    // size_t lower_b = G.ccOffset[G.ccRank[s]], upper_b = G.ccOffset[G.ccRank[s] + 1];
    // if (G.level[G.layerOffset[lower_b]] == 0)
    //     lower_b++;
...
void sssp_correctness_verifier(EdgeTy *cor_dist, EdgeTy *ch_dist, const PchGraph &GC, bool test = false)
{
    if (!test)
    {
    ...
    else
    {
        ...
        {
            if (cor_dist[i] != ch_dist[i] && (GC.level[i] > layer || GC.level[i] == 0))
            {
                layer = GC.level[i];
                itr++;
                cout << GC.level[i] << " cor_dist[" << i << "]=" << cor_dist[i] << ", my_dist[" << i << "]=" << ch_dist[i] << endl;
                for (size_t j = GC.offset[i]; j < GC.offset[i + 1]; j++)
                {
                    NodeId v = GC.E[j].v;
                    EdgeTy w = GC.E[j].w;
                    cout << GC.level[v] << " Neighbour " << v << " " << w << " "
                         << cor_dist[v] << " " << ch_dist[v] << endl;
                }
                if (itr == 300)
                {
                    assert(cor_dist[i] == ch_dist[GC.rank[i]]);
...
void sssp_verifier(const Graph &G, const PchGraph &GC, int source_num = 1, bool verify = true)
{
    ...
    for (int v = 0; v < source_num; ++v)
    {
        ...
        printf("%d\t%f\t%f\t%f\n", s, t1.total_time(), t2.total_time(), t3.total_time());
    }
    delete[] cor_dist;
    delete[] ch_dist;
}
